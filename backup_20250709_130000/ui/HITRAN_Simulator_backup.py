...
def show_sidebar():
    ...
    wl_min = st.sidebar.number_input("파장 최소 (nm)", value=1390.0, min_value=100.0, max_value=50000.0)
    wl_max = st.sidebar.number_input("파장 최대 (nm)", value=1395.0, min_value=100.0, max_value=50000.0)
    ...
    # 원자별 동위원소 표기 자동 변환
    import re
    def parse_isotope_formula(base, iso_code):
        # 예: base=CO2, iso_code=636 -> [("C", 13), ("O", 16), ("O", 18)]
        # 기본적으로 HITRAN iso_code는 각 원자 질량수의 조합임
        # base: ex) H2O, CO2, CH4, O3 등
        # iso_code: ex) 161, 636, 626, 166 등
        # base에서 원자 기호와 개수 추출
        atoms = re.findall(r"([A-Z][a-z]?)(\d*)", base)
        iso_digits = list(str(iso_code))
        result = []
        for atom, count in atoms:
            n = int(count) if count else 1
            for i in range(n):
                # 질량수는 2자리일 수 있으므로 남은 iso_digits 길이로 판단
                if len(iso_digits) >= 2 and int("".join(iso_digits[:2])) >= 10:
                    mass = int("".join([iso_digits.pop(0), iso_digits.pop(0)]))
                else:
                    mass = int(iso_digits.pop(0)) if iso_digits else None
                result.append((atom, mass))
        # 같은 원자끼리 묶어서 표기
        from collections import defaultdict
        grouped = defaultdict(list)
        for atom, mass in result:
            grouped[atom].append(mass)
        label = ""
        for atom, masses in atoms:
            a = atom
            count = int(masses) if masses else 1
            # 해당 원자에 할당된 질량수 리스트
            ms = grouped[a][:count]
            grouped[a] = grouped[a][count:]
            # 질량수(위첨자)는 한 번만, 개수는 아래첨자
            if ms and ms[0] is not None:
                label += to_superscript(str(ms[0]))
            label += a
            if count > 1:
                label += to_subscript(str(count))
        return label
    ...
    # 주요 분자 리스트(굵게 표시)
    MAJOR_MOLECULES = {"H2O", "CO2", "CH4", "N2O", "CO", "O3"}

    # 커스텀 CSS 삽입 (글자 크기 크게) → 제거
    # st.markdown("""
    #     <style>
    #     /* 멀티셀렉트 옵션 글자 크기 */
    ...
    def mol_label(key):
        info = HITRAN_MOLECULES[key]
        label = f"{info['kor']} ({isotope_label(info['base'], info['iso_code'])})"
        if info['base'] in MAJOR_MOLECULES:
            return f"<b>{label}</b>"
        return label
    # base별 그룹핑
    from collections import defaultdict
    ...
    if mode == "🧪 혼합 스펙트럼":
        selected_bases = st.sidebar.multiselect("분자 선택 (여러 개 선택 가능)", base_molecules, default=["H2O", "CO2", "CH4"], format_func=lambda b: f"{base_kor_map[b]} ({b})")
        # 각 분자별 농도 입력(ppb)
        base_concentrations = {}
    ...
    if mode == "🧪 혼합 스펙트럼":
        for base in selected_bases:
            ...
            selected = st.sidebar.multiselect(
                ...
                help="여러 동위원소를 동시에 선택할 수 있습니다. 대표 동위원소는 별표와 [대표]로 강조됩니다."
            )
            for iso in selected:
    ...
    return spectrometer_mode, mode, temp, wl_min, wl_max, molecules, molecule, c_min, c_max, c_steps, oa_icos_params, path, matrix_gas_params, molecule_concentrations

def run_simulation(spectrometer_mode, mode, temp, wl_min, wl_max, molecules, molecule=None, c_min=None, c_max=None, c_steps=None, oa_icos_params=None, path=1000.0, matrix_gas_params=None, molecule_concentrations=None):
    ...
    if mode == "🧪 혼합 스펙트럼" and molecules:
        ...
        for mol in molecules:
            ...
            if data is not None and len(data) > 0:
                # 농도 매핑 dict를 concentration 인자로 전달
                spec = calc.calculate_absorption_spectrum(
                    data, freq_grid, temp, pressure_atm, molecule_concentrations, path, mol
                )
        ...
        if data is not None and len(data) > 0:
            ...
            if oa_icos_sim and oa_icos_params and oa_icos_max_abs:
...
def show_results(results, wl_grid, mode, molecule=None, spectrometer_mode=None, oa_icos_params=None, matrix_gas_params=None, molecule_concentrations=None):
    ...
    if mode == "🧪 혼합 스펙트럼":
        ...
        st.subheader(f"📈 {molecule} 농도별 분석 결과")
...
def main():
    ...
    st.title("🔬 HITRAN CRDS Simulator (리팩토링 버전)")

    spectrometer_mode, mode, temp, wl_min, wl_max, molecules, molecule, c_min, c_max, c_steps, oa_icos_params, path, matrix_gas_params, molecule_concentrations = show_sidebar()
    # 설정 요약 한 줄로 표시
    summary = f"모드: {spectrometer_mode}, 분석: {mode}, 온도: {temp}K, 파장: {wl_min}~{wl_max} nm, 경로: {path:.1f}m"
    if oa_icos_params:
    ...
    if st.button(btn_text, type="primary"):
        with st.spinner('계산 중...'):
            results, wl_grid = run_simulation(spectrometer_mode, mode, temp, wl_min, wl_max, molecules, molecule, c_min, c_max, c_steps, oa_icos_params, path, matrix_gas_params, molecule_concentrations)
    ...
    # 파장 바로가기 기능 완전 제거
    # show_wavelength_shortcuts() 호출 및 함수 정의 삭제
